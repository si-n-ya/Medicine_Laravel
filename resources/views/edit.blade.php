    ...
    <main class="container form_container margin_top">
        <div class="flex_container">
            ...
            <p class="back_box btn_right">
                <a href="{{ action('ItemsController@list') }}" class="btn back">登録リストへ戻る</a>
            </p>
        </div> <!-- .flex_container -->
        <form action="{{ action('ItemsController@update', $form->id) }}" method="post" class="form margin_top">
            {{ csrf_field() }}
            <!-- 薬の名前 -->
            <div class="d_container flexiblebox">
                <dt class="dt conf_dt_bg"><label for="name">薬の名前</label></dt>
                <dd class="dd">
                    <input type="text" name="name" id="name" class="input_text" value="{{ old('name', $form->name) }}">
                    @if ($errors->has('name'))
                    {{ $errors->first('name') }}
                    @endif
                </dd>
            ...
            <!-- 服用する曜日 -->
            <div class="d_container flexiblebox">
                <dt class="dt conf_dt_bg">服用する曜日</dt>
                <dd class="dd week_dd">
                    <!-- 薬の編集 -->
                    @php
                    $week = ['日', '月', '火', '水', '木', '金', '土'];
                    ...
                    @for ($i = 0; $i < 7; $i++) <div class="check_layout">
                        <input type="checkbox" name="week[]" id="{{ $week[$i] }}" class="regist_week_check"
                            value="{{ $i }}"
                            {{ is_array(old('week', $get_week)) && in_array($i, old('week', $get_week)) ? 'checked': '' }}>
                        <label for="{{ $week[$i] }}">{{ $week[$i] }}曜
                        </label>
            </div> <!-- .check_layout -->
            @endfor
            ...
            <!-- 服用する時刻 -->
            <div class="d_container flexiblebox">
                <dt class="dt conf_dt_bg">服用する時刻</dt>
                <dd class="dd">
                    <!-- 薬の編集 -->
                    @for ($i = 0; $i < 24; $i++) <div class="check_layout">
                        <input type="checkbox" name="time[]" id="{{ $i }}" value="{{ $i }}"
                            {{ is_array(old('time', $get_time)) && in_array($i, old('time', $get_time)) ? 'checked': '' }}>
            ...
            <!-- 薬を使い始める日付 -->
            <div class="d_container flexiblebox">
                <dt class="dt conf_dt_bg"><label for="get_date">薬を使い始める日付</label>
                </dt>
                <dd class="dd">
                    <input type="date" id="get_date" class="input_text" name="get_date"
                        value="{{ old('get_date', $form->get_date) }}">
            ...
            <!-- 一回に服用する量 -->
            <div class="d_container flexiblebox">
                <dt class="dt conf_dt_bg"><label for="use_num">一回に服用する量</label></dt>
                <dd class="dd">
                    <!-- 薬の編集 -->
                    <input type="text" name="use_num" class="input_text" value="{{ old('use_num', $form->use_num) }}">
            ...
            <div class="flexiblebox">
                <dt class="dt conf_dt_bg"><label for="stock_num">在庫数</label></dt>
                <dd class="dd">
                    <input type="text" name="stock_num" id="stock" class="input_text" placeholder="(例) 60"
                        value="{{ old('stock_num', $form->stock_num) }}">
